...
class settingModule 
{
	...
	{		
		...
		$request_data['act'] = "check_rz";
		$request_data['id'] = $user_info['id'];
		$address_var = json_decode(call_api($request_data),true);
		$renzheng_status = $address_var['renzheng_status'];
		$GLOBALS['tmpl']->assign("renzheng_status",$renzheng_status);
	...
	public function smrz()
	{
		$user_info = $GLOBALS['user_info'];
		$GLOBALS['tmpl']->assign("user_info",$user_info);
		$GLOBALS['tmpl']->display("setting/smrz.html");
		#echo json_encode($GLOBALS['tmpl']);
	}
}
