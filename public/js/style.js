...
function submit(e){
	...
	obj.parent().prev().removeClass("none");
...
function search(){
	...
	var nick = $(".search_nick").val();
	var p_user_id = $(".search_pId").val();
	var d = {"id":id,"nick":nick,"p_user_id":p_user_id};
	$.ajax({
		type:"post",
		url:"m.php?ctl=user",
		data:d,
	...
	}else if(nick != "" && nick != undefined){
		window.location.href="m.php?ctl=user&nick="+nick;
	}else if(p_user_id != "" && p_user_id != undefined){
		window.location.href="m.php?ctl=user&p_user_id="+p_user_id;
	}else if(name != "" && name != undefined){
		window.location.href="m.php?ctl=user&real_name="+name;
	}else{
...
function searchAgency(){
	...
	var p_user_id = $(".search_pId").val();
	var packages = $(".search_package").val();
	var d = {"id":id,"nick":nick,"p_user_id":p_user_id,"real_name":name,"meal_id":packages,"status":status};
	$.ajax({
		type:"post",
		url:"m.php?ctl=proxy",
		data:d,
	...
	if(id != "" && id != undefined){
		window.location.href="m.php?ctl=proxy&id="+id;
	}else if(nick != "" && nick != undefined){
		window.location.href="m.php?ctl=proxy&nick="+nick;
	}else if(p_user_id != "" && p_user_id != undefined){
		window.location.href="m.php?ctl=proxy&p_user_id="+p_user_id;
	}else if(name != "" && name != undefined){
		window.location.href="m.php?ctl=proxy&real_name="+name;
	}else if(packages != "" && packages != undefined){
		window.location.href="m.php?ctl=proxy&meal_id="+packages;
	}else if(status != "" && status != undefined){
		window.location.href="m.php?ctl=proxy&status="+status;
	}else{
		window.location.href="m.php?ctl=proxy";
	}
}
...
function go_fh(e){
	...
	if(express_num =="不需要配送" || express_num != ""){
		$.ajax({
			type:"post",
...
function sp_search(e){
	...
	if(search_title == ""&&status == ""){
		return false;
	}else if(search_title != ""|| status != ""){
		window.location.href="m.php?ctl=order&order_status="+status+"&"+search_title+"="+search_con;
	}else if(search_title == ""|| status != ""){
		window.location.href="m.php?ctl=order&order_status="+status;
	}else if(search_title != ""|| status == ""){
		window.location.href="m.php?ctl=order&search_title="+search_con;
	}
}
